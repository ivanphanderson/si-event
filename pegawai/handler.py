...
class ValidColumnNameChecker(DataHandler):
    ...
    def __init__(self, next_handler):
        self.next_handler: DataHandler = next_handler
        self.valid_columns = [
            'no', 'employee no.', 'employee name', 'employee category',
            'job status', 'grade level', 'employment status', 'email',
            'nama di rekening', 'nama bank', 'no rekening', 'nomor npwp',
            'alamat npwp',
        ]
    ...
    def functionality(self, data: List[List[str]], information: Dict[str, str]):
        ...
        for i in range(len(header_row)):
            if header_row[i].lower() != self.valid_columns[i]:
                raise TypeError(f"*Terdeteksi kolom {header_row[i]} tidak sesuai, bisa posisi atau namanya!")
