...
def get_events(request, success_message=None):
  ...
  if success_message is not None:
    return render(request, EVENT_LIST, {'event_data':event_data, 'owner_data':owner_data, 'success_message':success_message, 'role': account.role})
  
  return render(request, EVENT_LIST, {'event_data':event_data, 'owner_data':owner_data, 'role': account.role})
...
def process_employee_data(event, form_data, idx):
    ...
    if str(employee_no).isnumeric():
        pegawai = Pegawai.objects.get(employee_no=employee_no)
        EventEmployee.objects.create(employee=pegawai, event=event, honor=honor, pph=pph, role=role)
...
def update_employee_to_event_by_id(request,id):
  ...
  if id.isdigit() and EventEmployee.objects.filter(id=id).first():
    ...
    messages.success(request, f'{event_employee.employee.employee_name} is updated successfully.')
...
def delete_event_employee_by_id(request, id):
  ...
  if id.isdigit() and EventEmployee.objects.filter(id=id).first():
    ...
    messages.success(request, f'{event_employee.employee.employee_name} is deleted successfully.')
    return redirect(f'/event/detail/{event.id}')
  return redirect(FORBIDDEN_URL)    