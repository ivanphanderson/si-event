...
from .views import read_akun, update_akun, ubah_password
import environ

env = environ.Env()
environ.Env.read_env()

REVERSE_ACC_REGIST = reverse("account:registerAccount")
...
ACCOUNT_URL = reverse("account:read_akun")
GANTI_STATUS_AKUN_URL = reverse("account:deactivate_akun")
UBAH_PASSWORD_URL = reverse("account:ubah_password")
UBAH_PASSWORD_SUBMIT_URL = reverse("account:submit_ubah_password")
FORBIDDEN_URL = reverse("home:forbidden")
REVERSE_HOME_HOME = reverse("home:home")
...
def create_test_users():
    ...
    Account.objects.create(
        user=admin_user, username="test_admin", email=TEST_ADMIN_EMAIL, role="Admin"
    )
    ...
    user_user.save()
    Account.objects.create(
        user=user_user, username="test_user", email=TEST_USER_EMAIL, role="User"
    )
    ...
    sk_user.save()
    Account.objects.create(
        user=sk_user, username="test_sk", email=TEST_SK_EMAIL, role="Staff Keuangan"
    )
...
class RegisterAccountViewTest(TestCase):
    ...
    def test_register_account_user_view_with_authenticated_admin_user(self):
        self.client.login(username=TEST_ADMIN_USERNAME, password=TEST_ADMIN_PASS)
        response = self.client.get(REVERSE_ACC_REGIST)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, REGIST_HTML)

        # submit valid form data
        response = self.client.post(REVERSE_ACC_REGIST, data=self.form_data_user)
    ...
    def test_register_account_admin_view_with_authenticated_admin_user(self):
        ...
        # submit valid form data
        response = self.client.post(REVERSE_ACC_REGIST, data=self.form_data_admin)
        self.assertEqual(response.status_code, 302)
        self.assertEqual(len(Account.objects.all()), 4)  # check if account was created
        self.assertTrue(
    ...
    def test_register_account_view_with_authenticated_non_admin_user(self):
        self.client.login(username=TEST_USER_USERNAME, password=TEST_USER_PASS)
        response = self.client.get(REVERSE_ACC_REGIST)
        self.assertEqual(response.status_code, 302)
    ...
    def test_register_account_view_with_invalid_form(self):
        ...
        response = self.client.post(REVERSE_ACC_REGIST, data=self.form_invalid)
        self.assertEqual(response.status_code, 200)
    ...
    def test_register_account_view_with_non_unique_username(self):
        ...
        # submit invalid form data
        response = self.client.post(REVERSE_ACC_REGIST, data=self.form_username_invalid)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(Account.objects.all()), 3)  # check if account was created
    ...
    def test_register_account_view_with_non_unique_email(self):
        ...
        # submit invalid form data
        response = self.client.post(REVERSE_ACC_REGIST, data=self.form_email_invalid)
        # print(response)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(Account.objects.all()), 3)  # check if account was created
        self.assertFalse(
            User.objects.filter(username="testuserr").exists()
        )  # check if user was created
...
def set_up_login(self, role):
    ...
    self.account = Account(
        user=user, username=self.USERNAME2, email="tes@gmail.com", role=role
    )
...
def set_up_akun_dummy(self):
    ...
    account = Account(user=user, username=self.USERNAME, email=self.EMAIL, role="Admin")
...
class UbahPasswordSudahLoginTest(TestCase):
    ...
    def test_ubah_password_post_password_invalid_dengan_kriteria_django_(self):
        ...
        self.assertTrue(
            updated_user.check_password(PASSWORD_UNTUK_TEST)
        )  # Password masih sama dengan password lama
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, HALAMAN_UBAH_PASSWORD_LOGGED_IN_HTML)
...
class AccountTidakLoginTest(TestCase):
    def test_ubah_password_is_exist(self):
        ...
        self.assertRedirects(
            response,
            f"/login?next=/account/ubah-password",
            status_code=302,
    ...
    def test_read_akun_is_exist(self):
        ...
        # karena belum login, diarahkan ke halaman login
        self.assertRedirects(
            response, f"/login?next=/account/", status_code=302, target_status_code=200
        )
    ...
    def test_ganti_status_akun_is_exist(self):
        ...
        self.assertRedirects(
            response,
            f"/login?next=/account/ganti-status-akun",
            status_code=302,
            target_status_code=200,
        )
...
class AccountSudahLoginAdminTest(TestCase):
    ...
    def test_admin_update_akun_post_is_exist(self):
        data = {"id_akun": self.account_dummy.id, "role": "User"}
        response = self.client.post(f"/account/update/submit/submit", data)
        self.assertRedirects(
            response, ACCOUNT_URL, status_code=302, target_status_code=200
        )
    ...
    def test_admin_update_akun_post_akun_sendiri_tidak_bisa(self):
        data = {"id_akun": self.account.id, "role": "User"}
        response = self.client.post(f"/account/update/submit/submit", data)
        self.assertRedirects(
            response, FORBIDDEN_URL, status_code=302, target_status_code=200
        )
    ...
    def test_admin_ganti_status_akun_is_exist(self):
        response = self.client.post(
            GANTI_STATUS_AKUN_URL, {"id_akun": self.account_dummy.id}
        )
