...
UPDATE_AKUN_URL_ASAL = "/account/update/3333"
UPDATE_AKUN_NOT_INT_ID_URL = "/account/update/aasdd"
SUBMIT_AKUN_UPDATE_URL = "/account/update/submit/submit"
...
def set_up_login(self, role):
    ...
    non_sso_account = NonSSOAccount(
        user=user, username=self.USERNAME2, email=TEST_EMAIL, role=role
    )
...
class AccountTidakLoginTest(TestCase):
    def test_ubah_password_is_exist(self):
        ...
        self.assertRedirects(
            response,
            "/login?next=/account/ubah-password",
            status_code=302,
    ...
    def test_read_akun_is_exist(self):
        ...
        # karena belum login, diarahkan ke halaman login
        self.assertRedirects(
            response, "/login?next=/account/", status_code=302, target_status_code=200
        )
    ...
    def test_ganti_status_akun_is_exist(self):
        ...
        self.assertRedirects(
            response,
            "/login?next=/account/ganti-status-akun",
            status_code=302,
            target_status_code=200,
        )
...
class AccountSudahLoginAdminTest(TestCase):
    ...
    def test_admin_update_akun_id_ngasal(self):
        response = self.client.get(UPDATE_AKUN_URL_ASAL)
        self.assertRedirects(
    ...
    def test_admin_update_akun_id_bukan_int(self):
        response = self.client.get(UPDATE_AKUN_NOT_INT_ID_URL)
        self.assertRedirects(
            response, FORBIDDEN_URL, status_code=302, target_status_code=200
        )
    ...
    def test_admin_update_akun_post_is_exist(self):
        ...
        response = self.client.post(SUBMIT_AKUN_UPDATE_URL, data)
        self.assertRedirects(
    ...
    def test_admin_update_akun_post_sso_is_exist(self):
        data = {"id_akun": self.account_dummy2.id, "role": "User"}
        response = self.client.post(SUBMIT_AKUN_UPDATE_URL, data)
        self.assertRedirects(
            response, ACCOUNT_URL, status_code=302, target_status_code=200
        )
...
class AccountModelTestCase(TestCase):
    def test_non_sso_ui_account_str_representation(self):
        ...
        account = NonSSOAccount.objects.create(
            user=user, username="tesuser", email="tesuser@gmail.com", role="Admin"
        )
