...
URL= '/pembayaran/'
DOWNLOAD_EXCEL_STD_URL = reverse('pembayaran:download_as_excel', kwargs={'type': 'standard'})
DOWNLOAD_EXCEL_BTRMEMO_URL = reverse('pembayaran:download_as_excel', kwargs={'type': 'btrmemo'})
...
class DisplayPembayaranNotAuthenticatedTest(TestCase):
    ...
    def test_read_pembayaran_not_authenticated(self):
        ...
        self.assertRedirects(response, '/login?next=/pembayaran/', status_code=302, target_status_code=200)
...
class DownloadExcelFromData(TestCase):
    ...
    def test_download_std_excel(self):
        response = self.client.post(DOWNLOAD_EXCEL_STD_URL, json.dumps(self.data_table), content_type="application/json")

        excel_file = BytesIO(response.content)
        excel_file_content = pd.read_excel(excel_file)

        self.assertEqual(response.status_code, 200)
        self.assertIn('filename=data pembayaran panitia event.xlsx', response['Content-Dispotition'])
        self.assertEquals(excel_file_content.shape, (2, 9))
    
    def test_download_btrmemo_excel(self):
        response = self.client.post(DOWNLOAD_EXCEL_BTRMEMO_URL, json.dumps(self.data_table), content_type="application/json")
