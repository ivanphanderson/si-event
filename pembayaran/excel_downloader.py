    ...
    def get_excel(self, table_data):
        ...
        memo.merge_range(
            ...
            cell_format=workbook.add_format(
                {"bold": True, "font_size": 22, "bg_color": "gray"}
            ),
        )
        memo.write_string(
            7,
        ...
        memo.write_string(
            21, ws_col, "Demikian  kami sampaikan, mohon persetujuannya. Terima kasih."
        )
        ...
        for row_accessor in table_data.keys():
            ...
            for col_accessor in row_data.keys():
                ...
                if col_accessor == "total":
                    daftar_pembayaran.merge_range(
                        f"A{ws_row+1}:C{ws_row+1}", row_data[col_accessor], cell_format
                    )
                else:
                    daftar_pembayaran.write_string(
                        ws_row, ws_col, row_data[col_accessor], cell_format
                    )
                ws_col += 1
            ws_row += 1
            ws_col = 0
        ...
        daftar_pembayaran.write_string(
            ws_row + 11, ws_col + 1, "(Hennie Marianie, M.SM.)"
        )
...
def excel_factory(type="standard"):
    types = {"standard": StandardExcel, "btrmemo": BTRMemoExcel}
