...
FORGET_PASSWORD_URL = reverse("authentication:forget_password")
FORGET_PASSWORD_URL_SUBMIT = reverse("authentication:submit_forget_password")
OTP_URL_SUBMIT = reverse("authentication:submit_otp")
UBAH_PASSWORD_URL_SUBMIT = reverse("authentication:submit_ubah_password")
...
UBAH_PASSWORD_URL_LENGKAP_USERNAME = (
    "https://si-event.herokuapp.com/ubah-password/tesname2"
)
UBAH_PASSWORD_URL_LENGKAP_SUBMIT = (
    "https://si-event.herokuapp.com/ubah-password/submit/submit"
)
...
class LoginLogoutTest(TestCase):
    ...
    def test_login_admin_dua_kali_success_to_home(self):
        ...
        logged_in = self.client.login(
            username=self.ADMIN_USERNAME, password=PASSWORD_UNTUK_TEST_GANTI
        )
        self.assertEqual(logged_in, True)
        response = self.client.post(
            reverse(REVERSE_AUTH_LOGIN),
    ...
    def test_login_admin_failed(self):
        ...
        response = self.client.post(
            url, {"username": "admin_unregistered", "password": "p4ssw0rdd"}
        )
    ...
    def test_login_user_dua_kali_success_to_home(self):
        ...
        logged_in = self.client.login(
            username=self.USER_USERNAME, password=PASSWORD_UNTUK_TEST_GANTI
        )
        self.assertEqual(logged_in, True)
        response = self.client.post(
            reverse(REVERSE_AUTH_LOGIN),
    ...
    def test_login_user_failed(self):
        url = reverse(REVERSE_AUTH_LOGIN)
        response = self.client.post(
            url, {"username": "user_unregistered", "password": "p4ssw0rdd"}
        )
        self.assertFalse(response.context["user"].is_authenticated)
    ...
    def test_login_sk_success(self):
        logged_in = self.client.login(
            username=self.STAFF_KEUANGAN_USERNAME, password=self.STAFF_KEUANGAN_PASSWORD
        )
        self.assertEqual(logged_in, True)
        response = self.client.post(
            reverse(REVERSE_AUTH_LOGIN),
            {
                "username": self.STAFF_KEUANGAN_USERNAME,
                "password": self.STAFF_KEUANGAN_PASSWORD,
            },
            follow=True,
        )
        self.assertTrue(response.context["user"].is_authenticated)
    ...
    def test_login_sk_dua_kali_success_to_home(self):
        ...
        logged_in = self.client.login(
            username=self.STAFF_KEUANGAN_USERNAME, password=PASSWORD_UNTUK_TEST_GANTI
        )
        self.assertEqual(logged_in, True)
        response = self.client.post(
            reverse(REVERSE_AUTH_LOGIN),
    ...
    def test_login_sk_failed(self):
        url = reverse(REVERSE_AUTH_LOGIN)
        response = self.client.post(
            url, {"username": "sk_unregistered", "password": "p4ssw0rdd"}
        )
        self.assertFalse(response.context["user"].is_authenticated)
...
class ModelTest(TestCase):
    def setUp(self):
        ...
        self.OTP = 12345678
        PasswordOTP.objects.create(username=self.USERNAME, OTP=self.OTP)
    ...
    def test_model_password_otp(self):
        ...
        self.assertEqual(
            password_otp.is_redeem, False
        )  # False karena belum dipakai dimana-mana
        self.assertEqual(
            password_otp.is_changed, False
        )  # False karena belum dipakai dimana-mana
...
class LupaPasswordTest(TestCase):
    def setUp(self):
        ...
        account = Account(
            user=user, username=self.USERNAME, email=self.EMAIL, role="Admin"
        )
    ...
    def test_forget_password_post(self):
        ...
        self.assertRedirects(
            response, f"/otp/{self.USERNAME}", status_code=302, target_status_code=200
        )
    ...
    def test_forget_password_post_invalid_data(self):
        data = {"username": self.USERNAME, "email": "asdsa@gmail.com"}
    ...
    def test_otp_post_is_functional(self):
        ...
        data2 = {"OTP": otp, "username": self.USERNAME}

        response = Client().post(OTP_URL_SUBMIT, data2)
        self.assertRedirects(
            response,
            UBAH_PASSWORD_URL_USERNAME,
            status_code=302,
    ...
    def test_otp_post_otp_salah(self):
        ...
        data2 = {"OTP": 000000, "username": self.USERNAME}
    ...
    def test_ubah_password_get_is_functional_http_referer_versi_submit_password(self):
        ...
        headers = {
            "HTTP_REFERER": "https://si-event.herokuapp.com/ubah-password/submit/submit"
        }
        response = Client().get(UBAH_PASSWORD_URL_USERNAME, {}, **headers)
    ...
    def test_ubah_password_post_is_functional(self):
        ...
        self.assertRedirects(
            response, f"/login", status_code=302, target_status_code=200
        )
